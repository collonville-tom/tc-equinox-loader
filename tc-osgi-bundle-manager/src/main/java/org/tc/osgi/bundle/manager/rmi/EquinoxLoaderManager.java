...
public class EquinoxLoaderManager implements IRPCServer {
    ...
    public void addObject(final String signature, final Object obj) throws RemoteException, MalformedURLException, UnknownHostException,
        ...
        if (obj instanceof Remote) {
            LoggerServiceProxy.getInstance().getLogger(EquinoxLoaderManager.class).debug("Enregistrement de l'objet accessible a l'url : "
                + url);
    ...
    private <T> ObjectName buildObjectName(final T object) throws TcManagerMBeanException {
        final Class<?> c = object.getClass();
        final String objectName = c.getPackage().getName() + ":type=" + c.getSimpleName();
    ...
    public void listRMIRegistry() throws AccessException, RemoteException {
        if (registre != null) {
            final StringBuilder b = new StringBuilder("Liste des services RMI actif").append("\n");
            for (final String s : registre.list()) {
                b.append(s).append("\n");
            }
    ...
    public <T> void register(final T object, final Class signature) throws TcManagerMBeanException {
        try {
            // on l'ajoute au server de bean
            server.registerMBean(object, this.buildObjectName(object));

            // ensuite on l'expose en RMI
            addObject(signature.getSimpleName(), object);

        } catch (NotCompliantMBeanException | MBeanRegistrationException | InstanceAlreadyExistsException | TcManagerMBeanException
            | RemoteException | MalformedURLException | UnknownHostException | FieldTrackingAssignementException e) {
            throw new TcManagerMBeanException("Impossible d enregistrer le bean " + object.getClass(), e);
        }
    }

    public <T> void unRegister(final T object, final Class signature) throws TcManagerMBeanException {
        try {
            server.unregisterMBean(this.buildObjectName(object));
            registre.unbind(signature.getSimpleName());
