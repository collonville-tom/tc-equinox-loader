...
// registre des repository distant, permet de consolider l'ensmeble des sources
// de bundles sous le format tar-gz,
// et facilite la consulation l'import et l'installation y compris le repo local
// qui est une sorte de remote repo mais en local
public class RemoteRegistry implements RemoteRegistryMBean {
    ...
    @Override
    public String addRepo(final String name, final String url) {
    ...
    @Override
    public String delRepo(final String name) {
        if (!RepositoryManager.getRepositoryManager().getRepositories().containsKey(name)) {
    ...
    @Override
    public String deployTar(final String bundleName, final String version) {
        if (!System.getProperty(RemoteRegistry.OS_PROPERTY).toLowerCase().startsWith(RemoteRegistry.WINDOWS)) {
            final StringBuilder b = new StringBuilder(RemoteRegistry.TAR_CMD);
            b.append(" -f ").append(RemoteRegistry.LOCAL_WORK_DIR);
            b.append(bundleName).append("-");
            b.append(version).append(RemoteRegistry.ARCH_EXT);
            b.append(" -C /");
            try {
                LoggerServiceProxy.getInstance().getLogger(RemoteRegistry.class).info("Extractiong" + bundleName);
                LoggerServiceProxy.getInstance().getLogger(RemoteRegistry.class).debug("Extractiong CMD" + b.toString());
                final Process process = Runtime.getRuntime().exec(b.toString());
                process.waitFor();
                if (process.exitValue() != 0) {
                    return "Erreur pendant d'extraction de l'archive";
                }
                return new StringBuilder("Extracting ").append(bundleName).append("-").append(version).append(".tar.gz done").toString();
    ...
    public String fetchRemoteRepo() {
        ...
        for (final RemoteRepository r : RepositoryManager.getRepositoryManager().getRepositories().values()) {
            r.fetch();
    ...
    private String find(final String tarname) throws DownloaderException {
        for (final RemoteRepository r : RepositoryManager.getRepositoryManager().getRepositories().values()) {
            for (final ITarGzBundle bundle : r.getBundles()) {
    ...
    @Override
    public String pullTar(final String tarname, final String version) {
        ...
        try {
            ...
            d.downloadFile(url, new StringBuilder(RemoteRegistry.LOCAL_WORK_DIR).append(tarname).append("-").append(version).append(
                RemoteRegistry.ARCH_EXT).toString());
    ...
    @Override
    public String pushTar(final String name, final String version) {
        for (final ITarGzBundle tgz : RepositoryManager.getRepositoryManager().getLocalRepository().getBundles()) {
            if (tgz.getName().equals(name) && tgz.getVersion().equals(version)) {
                final StringBuilder b = new StringBuilder("/local/");
                b.append(name).append("-").append(version).append(RemoteRegistry.ARCH_EXT);
                return b.toString();
            }
        }
    ...
    public String toString() {
        final StringBuilder b = new StringBuilder("Repositories:\n");
        for (final RemoteRepository r : RepositoryManager.getRepositoryManager().getRepositories().values()) {
